...
//		y = y + 5;
		...
		// mousestate version of mouse support
		// https://www.libsdl.org/release/SDL-1.2.15/docs/html/sdlgetmousestate.html
		mousestat  = SDL_GetMouseState(&mx, &my);
		paddle -> pos.x = mx;
		if ((mousestat & SDL_BUTTON(1)) == SDL_BUTTON(1)) // left button
			paddle -> pos.y = paddle -> pos.y - 5;
		if ((mousestat & SDL_BUTTON(3)) == SDL_BUTTON(3)) // left button
			paddle -> pos.y = paddle -> pos.y + 5;
