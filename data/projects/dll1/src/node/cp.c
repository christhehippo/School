...
//
//               the contents of the "left" and "right" pointers,
...
code_t cpnode(Node *oldNode, Node **copiedNode)
{
	...
	{
		codeCP = mknode(copiedNode, 0);
		(*copiedNode) -> left     = oldNode -> left;
		(*copiedNode) -> right    = oldNode -> right;
		(*copiedNode) -> payload  = oldNode -> payload;
		code = code | codeCP;
	}
        ...
        if ((*copiedNode)                  == NULL)
        {
                ...
                (oldNode                   != UNDEFINED))
            {
                ...
                if ((*copiedNode)          != NULL)
                {
                    ...
                    //////////////////////////////////////////////////
                    //
                    // As per project specifications, point copied 
                    // node "left" and "right" pointers to whatever
                    // the original node "left" and "right" pointers
