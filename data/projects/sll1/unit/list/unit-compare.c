...
int main()
{
    ...
    for (i = 0; i < 8; i++)
    {
        myList1 = insert(myList1, myList1 -> lead, mknode(data[i]));
        myList2 = insert(myList2, myList2 -> lead, mknode(data[i]));
    }
	...
	if ((myList2 != NULL) && (myList2 != UNDEFINED))
		myList2 -> lead -> right -> right -> right -> info = 37;
    result                    = compare(myList1, myList2, &pos);
    if (result               == CMP_EQUALITY)
        fprintf(stdout, " you have: EQUAL!\n");
    else
